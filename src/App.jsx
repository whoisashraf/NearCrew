...
const App = () => {
  ...
  const addService = async (newService) => {
    const res = await fetch('/api/services', { // Updated API endpoint
      method: 'POST',
      headers: {
  ...
  const deleteService = async (id) => {
    const res = await fetch(`/api/services/${id}`, { // Updated API endpoint
      method: 'DELETE',
    });
    return res.json(); // Return the response data if needed
  };
  ...
  const updateService = async (service) => {
    const res = await fetch(`/api/services/${service.id}`, { // Updated API endpoint
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const router = createBrowserRouter(
    createRoutesFromElements(
      <Route path='/' element={<MainLayout />}>
        ...
        <Route path='/services' element={<ServicesPage />} /> {/* Updated to ServicesPage */}
        <Route path='/add-service' element={<AddServicePage addServiceSubmit={addService} />} /> {/* Updated to AddServicePage */}
        <Route
